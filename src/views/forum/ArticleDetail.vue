<template>
    <div class="container-body article-detail-body" :style="{width:proxy.globalInfo.bodyWidth+'px'}" v-if="Object.keys(articleInfo).length > 0">
        <!-- 板块导航 -->
...
<script setup>
    ...
    const downloadAttachment = async (fileId) => {
        ...
        if(result.data.userIntegral < attachment.value.integral ){
            proxy.Message.warning("积分不够，无法下载");
            return;
        }
    ...
    const imagePreview = () => {
        nextTick(() => {
            ...
            imageNodeList.forEach((item,index) => {
                const src = item.getAttribute("src")//拿到每一张图片的src
                imageList.push(src);//传过去的是所有图片的src数组
                item.addEventListener('click',() => {
                    //当 当前图片 被点击时，它的下标就被传到了el-image-viewer组件中
                    imageViewerRef.value.show(index)    
                })
            })
            previewImgList.value = imageList;   //传过去了一个src数组
        })
    }
    ...
    const makeToc = () => {
        nextTick(() => {
            ...
            childNodes.forEach((item) => {
                ...
                index++;
                let id = "toc" + index;
                item.setAttribute("id",id);//给每一个目录加上一个id
                tocArray.value.push({
                    id:id,
...
<style lang="scss" scoped>
    ...
    .quick-panel {
        ...
        .quick-item {
            width: 50px;
            height: 50px;
            display: flex; //开启flex布局，默认在主轴左侧开始排列
